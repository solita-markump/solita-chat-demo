...
- [x] Initialize npm-based Vue + TypeScript project under `src\Frontend`.
- [x] Add Manifest V3 config and popup entry wiring so clicking the extension icon opens the popup.
- **Commit**: `scaffold vue extension workspace`

2. **Configure Tailwind for popup**
- [x] Install and configure Tailwind + PostCSS in the frontend project.
- [x] Add popup base size/layout styles for a consistent extension popup frame.
- **Commit**: `configure tailwind for extension popup`

3. **Build minimal popup chat draft**
- [x] Implement popup UI sections: header/title, mock message list, and composer (input + send button).
- [x] Keep functionality static (no API calls, auth, persistence, or realtime transport).
- **Commit**: `build minimal popup chat ui draft`

4. **Document frontend workflow**
- [x] Update `README.md` with install/build and Chrome load-unpacked instructions for the popup extension.
- [x] Update `architecture.md` frontend stack to Vue + Tailwind.
- **Commit**: `document frontend extension workflow`

5. **Validate draft**
- [x] Run frontend checks available from the scaffold (`build`, `type-check`, `lint` if present).
- [x] Run relevant existing backend checks to ensure no regressions from repo changes.
